...
if ($method === 'POST' && !$orderId) {
    ...
    if (empty($items)) {
        echo json_encode(['status' => 'error', 'message' => 'At least one mealbox required in items']); exit;
    }
    ...
    // Get all mealbox orders for vendor (filter only mealbox orders)
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Only include if all items are mealbox type
        $allMealbox = true;
    ...
    // Get single mealbox order (only if all items are mealbox)
